...
HEADERS = [
    ...
    "الفئة (اختياري)", "الفرع", "المجموعة (اختياري)", "اسم ولي الأمر (اختياري)", "جوال ولي الأمر (اختياري)",
...
def build_template_workbook(conn):
    ...
    lines = [
        ...
        "2) الحقول المطلوبة: الاسم الأول، اسم العائلة، الجنس، الفرع. تاريخ الميلاد والفئة اختياريان — اتركهما فارغين إذا ما كانا متوفرين (يمكن تعيين الفئة لاحقًا من ملف اللاعب).",
        "3) إذا عبّأت الفئة أو الفرع، اكتبهما بنفس الإملاء الموجود بالضبط في تبويب «الفئات والفروع والمجموعات» — أو اختر من القائمة المنسدلة في كل خلية. عمود المجموعة اختياري: اكتب اسم المجموعة فقط (عمود C في تبويب المرجع) بدون الفرع أو الفئة.",
...
def parse_workbook(conn, file_stream):
    ...
    for row_idx, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
        ...
        # تاريخ الميلاد اختياري — إذا تُرك فارغًا يُحفظ فارغًا بدون خطأ، لكن إذا
        # كُتب يجب أن يكون بصيغة صحيحة.
        dob = None
        ...
        # الفئة اختيارية — إذا تُركت فارغة يُسجَّل اللاعب بدون فئة (يمكن
        # تعيينها لاحقًا من ملفه الشخصي)، لكن إذا كُتبت يجب أن تكون فئة
        # موجودة فعليًا في النظام.
